...
import { DetailCashbackModal } from "./DetailCashback";
import { useModal } from "../action/modal";
import { UseCustomDisclosureReturn } from "@/hooks/useCustomDisclosure";
import Image from 'next/image'

export default function CashbackModal({ disclosure }: { disclosure: UseCustomDisclosureReturn }) {
  const detailCashback = useModal();

  const handleOpenDetailCommissionModal = () => {
    detailCashback.openModal();
  }

  return (
    <>
      <DetailCashbackModal isOpen={detailCashback.isOpen} onClose={detailCashback.closeModal} />
      ...
      >
        <ModalContent>
          <ModalHeader>คืนยอดเสีย</ModalHeader>
          <ModalBody>
            <div className="grid grid-cols-2 gap-3">
              <Card className="flex flex-col  items-center border border-white/20 py-4 px-3 gap-3">
                ...
                <Button className="Btn1 w-full">รับคอมมิชชั่น</Button>
              </Card>
              <Card className="flex flex-col  items-center border border-white/20 py-4 px-3 gap-3">
                <div className="flex flex-col items-center gap-3">
                  <Image
